...
8. 偏好共享工具包，不重复造轮子；智能体能力优先使用AgentScope
9. 所有命令实际执行，不空谈"应该可以"
10. 医疗安全底线：禁止确定性诊断，所有医疗输出带免责声明，高风险症状提示立即就医
11. 适老化无障碍：患者端老年模式≥18px正文、≥48px按钮、高对比度、图标必有文字标签
12. 配置不可硬编码：所有API Key/模型名/URL通过环境变量配置，.env.example提供模板
13. API Client层抽象：所有外部API调用走services/层，为二阶段迁移后端预留适配
14. 每次只做对应开发模块的必要测试，不做全量测试，只有在所有功能开发完毕后才做全量回归测试
