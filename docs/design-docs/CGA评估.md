...
5. **访客可直接完成评估**：访客通过伪匿名会话即可使用 CGA 评估；量表和结果按当前身份隔离，前端 localStorage 只承担交互缓存，流式对话和服务端事实源不因未登录而关闭
...
| mimo-v2.5-tts | Mimo API | 问题和报告语音朗读（使用服务商默认音色） | 重试2次→语音按钮显示禁用状态，提示"语音播放失败，您可以看文字"，不阻塞答题流程 |
...
当前实现中，访客和账号都通过 BFF 使用服务端事实源；localStorage 只保存交互缓存、未完成页面状态和界面偏好，key 命名规范仍可使用 `gerclaw:cga:{key}`，但不得把它当作权限或跨患者数据来源。
...
| `gerclaw:cga:completed_assessments` | `AssessmentReport[]` | 当前页面的评估交互缓存；访客关闭页面后不恢复，账号恢复以服务端记录和权限为准 |
...
- **数据安全**：
  - 访客评估按伪匿名会话隔离，服务端流式处理和保存遵循当前 actor 边界；前端缓存不作为事实源
  - localStorage 不保存用户真实身份信息；清除浏览器数据只清理本地缓存，不等同于删除服务端账号记录
...
- **医生端患者标识**：访客医生端只处理当前伪匿名会话中的自有评估，不显示跨患者列表；账号医生端才可在已授权范围内关联患者资料。
