...
系统始终先展示登录页；使用者可选择匿名进入患者端，但并非“无后端身份”。Next.js BFF 使用该页面生命周期内的随机 visitor ID，并以独立 `GERCLAW_GUEST_IDENTITY_SECRET` 签名；FastAPI 验签后派生伪匿名 `actor_id`，再签发短期、最小权限 JWT。浏览器 JavaScript 不接触 JWT、签名密钥或 Provider key，visitor ID 不会在下一次进入时恢复。

- 同一页面生命周期内的 BFF 请求稳定映射到同一匿名 actor；刷新或再次进入会生成新的匿名主体，前端不恢复游客历史会话。
...
- 访客可在患者端和医生端之间切换展示，并可使用所有自有对话、评估、处方、文件、语音、检索、Memory、Skill 和服务配置能力；端切换不产生医生资质，也不扩大数据授权。账号密码、MFA 和正式患者/医生 RBAC 仍由账号身份负责。
...
| 角色 | 权限范围 |
|------|---------|
| 伪匿名访客 | 与账号相同的自有产品 scope；可执行和管理自有 Skill、配置自有服务、使用患者/医生工作台；不可跨患者访问、授予患者授权、处理管理员功能或作出临床审批决策；历史仅限当前会话 |
| 患者/医生账号 | UI 角色来自服务端已验证账号，前端不能自行扩大后端 scope |
...
> **禁止在前端代码或 localStorage 中存储真实医疗身份信息；游客不持久化会话、visitor ID 或可恢复的历史。**
...
| 数据类型 | 分类 | 存储要求 | 传输要求 |
|---------|------|---------|---------|
| API密钥 | 机密 | 部署默认 Key 通过环境变量注入；用户配置经加密列按当前访客 actor 或账号保存，不写入前端代码或localStorage | HTTPS |
...
- 输出内容后处理：按完整句检测直接临床结论用语（如“您患有”“这是某疾病”“诊断是”“明确诊断为”）。仅当本轮没有可追溯 citation 与 Runtime evidence marker 时改写；有本地、受治理联网或当前上传资料证据时允许保留，`AgentResponse` 强制两者同时存在。患者端仅在整段末尾追加一次改药/复核风险提示，医生端不作机械改写；`SafetyDecision` 仅在确实发生无证据改写时标记 blocked。
