...
## 2026-07-18：Bad Case 七日趋势的 PostgreSQL 集成验证

当前 `test-api` 镜像在独立 Compose 测试数据库完成 Alembic 迁移后，实际执行
`tests/test_api_integration.py`，结果为 `8 passed`。新增断言调用管理员 Bad Case 趋势
接口，验证响应覆盖连续七日、只含日期/来源/计数的聚合值，并与该测试创建的合成
Bad Case 总数一致。接口查询和响应均不包含快照、反馈正文、图片、Trace、会话或用户
标识。

该结果验证迁移后的 PostgreSQL 路由和聚合契约，不是管理员页面浏览器 E2E、真实病例
去标识化、临床评测或发布验收证据。

## 2026-07-18：全量质量门禁后的 Compose 最终联调复验

证据文件：`perf-sse-safety-short-circuit-compose-2026-07-18-final.json`、
`perf-medication-review-workflow-compose-2026-07-18-final.json`

在 API 迁移处于 `a41c814f2045` head、`/health/ready` 全部通过，且完整 quality gate
通过后，分别以最多 10 个独立访客重跑两个受控确定性 workload。安全短路为 10/10 HTTP
200、10/10 SSE `done`、10 个唯一 completed Trace、失败率 0、p50 322ms、p95 323ms；访客
history 10/10 为 `403 GUEST_SESSION_HISTORY_DISABLED`，跨访客 Trace 为 404。用药审查为
10/10 HTTP 200、10 个唯一 completed Trace、失败率 0、p50 52ms、p95 55ms；全部返回
`medication-rules-v4` finding 和来源，跨访客 intake/Trace 均为 404。
