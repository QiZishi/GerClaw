...
- 在健康的 Docker Compose API、PostgreSQL、Redis、Qdrant 上，以最多 10 个并发会话执行受控、无外部模型成本的安全短路 SSE 路径；每个会话必须独立创建并完成 Trace。
- 报告请求总延迟 p50/p95、失败率、HTTP/SSE 终态、Trace 唯一性、访客 history 的强制拒绝与跨访客 Trace 隔离。不得把该安全短路工作负载表述为真实 LLM/RAG 吞吐或千级容量证明。
...
- 2026-07-18：访客 history 已改为整体不可读后，更新性能脚本为 `perf-sse-safety-short-circuit-v2`，避免把正确的 403 当作失败。当前 Compose API 上实际运行 10 并发：10/10 HTTP 200 与 SSE `done`、失败率 0、p50 99ms、p95 100ms、10 个唯一 completed Trace；10/10 history 返回 `403 GUEST_SESSION_HISTORY_DISABLED`，跨访客 Trace 返回 404。原始 PHI-free JSON 为 `docs/evidence/perf-sse-safety-short-circuit-compose-2026-07-18.json`。它仍不代表模型、RAG、临床 workflow 或千级容量。
- 2026-07-18：增加 `apps/api/scripts/perf_medication_review_workflow.py`，以最多 10 个独立访客准备独立用药审查 intake 后，仅并发执行真实 `medication-review-draft`。它强制校验 `medication-rules-v3`、finding/来源、各自 completed Trace 与跨访客 intake/Trace 404，不在报告回显合成输入。当前 Compose 重建镜像后实际运行结果为 10/10 HTTP 200、失败率 0、p50 50ms、p95 53ms、10 个唯一 completed Trace，规则 finding/来源均为 10/10；PHI-free 证据为 `docs/evidence/perf-medication-review-workflow-compose-2026-07-18.json`。这覆盖一个确定性临床工作流，不是临床正确性、外部模型/RAG/MinerU、五大处方长流程或千级容量证明。
- 2026-07-18：同次审查暴露 integration fixture 仍把默认患者 token 签为 guest，且三条测试仍期待旧的会话、Memory detail 或访客 Skill 契约。修正 fixture 和测试为显式角色/当前接口后，在 Compose 专用测试网络重新执行完整非 external 套件，结果 `698 passed, 10 deselected`、coverage `83.98%`；仅有 Qdrant HTTPS 和本地 payload-index 能力的已知 warning。
...
- 2026-07-18：扩充版本绑定的合成 RAG 检索基线为五个本地老年医学主题（多重用药、跌倒、衰弱、轻度认知损害、稳定性冠心病用药）和一个无证据例，并拒绝重复 case ID。以当前 test image、当前 immutable index、显式 `--allow-external-rag --top-k 5 --max-cases 6` 真实运行，6/6 通过；每个正例均命中绑定 document ID 且五条返回均具完整 citation provenance，非医学问题返回空。此运行会调用 embedding/rerank 服务，结果具非确定性；它只证明该有限检索回归契约，不代表临床有效性、模型质量、完整 RAG 评测或容量。
- 2026-07-18：新增老年焦虑与压力性损伤两条审核合成正例后，Compose 真实回归首次为 7/8。诊断显示中文焦虑查询的目标英文指南已在 30 条融合候选中（第 29），却被默认 20 条 rerank 窗口提前丢弃；不降低用例断言，而是将默认 `GERCLAW_RAG_RERANK_CANDIDATES` 调整为与 30 条召回窗口一致。公开语料以 `lexical-cjk-ngram-v2` 完整重建为 436/436 文档、39,837 chunks、0 失败后，显式 external embedding/rerank/Qdrant 真实执行 8/8 通过；机器可读的 PHI-free 聚合证据为 `docs/evidence/rag-retrieval-reviewed-compose-2026-07-18-v2.json`。这仍不代表临床有效性、全语料召回率或容量验收。
