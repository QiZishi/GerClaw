...
### 2026-08-02 线上失效账号 cookie 复测追加

ModelScope GUI 复测发现，`/api/account/status` 在失效账号 access cookie 时可以返回 HTTP 200 与 `{authenticated:false}`，但部署边缘层未可靠删除旧 HttpOnly cookie；之后访客入口的 `/sessions` 仍可能先带失效账号令牌并收到 401。该状态失败会被前端误呈现为对话失败。

修正为 BFF 只对这一次 401 做身份恢复：若上游拒绝当前账号令牌，强制签发同一 visitor ID 对应的访客令牌，重放原请求，并附带清理账号 cookie；模型输出、医疗文本和输入内容均不参与该恢复判断。这样账号 cookie 的边缘清理即使延迟，后续会话/对话也不会被旧账号令牌阻断。

隔离 Playwright 线上验证结果：
