...
- 登录账户刷新会话历史后，前端查询当前会话的 recoverable Run。实时 transport reconnect 从当前 UI 已渲染的最后 sequence 继续；页面 refresh 的 history hydration 没有渲染任何 RunEvent，因此 `running` Run 从 sequence 0 经 GET stream 完整重建，不能把 REST replay 的终态丢弃后订阅空流。`interrupted` Run 才显式 POST resume；检查期间已经终态化的 Run 重新读取数据库消息。新聊天、续传与恢复共用同一 SSE parser 和取消协议；游标按 Run ID 绑定、sequence 单调去重，跨 Run 或缺半边游标 fail closed。恢复中的 Run 只更新所属会话，不能劫持用户正在查看的其他会话。访客历史仍按既有边界不跨页面恢复。
...
- 每条已完成的 AI 回答首次点击“有帮助”或“没帮助”时统一打开反馈弹窗；评价文字为可选项、最多 2000 字，取消不得写入。评价文字在提交前仅存在于组件内存，禁止写入 `localStorage`；提交后仅以 PostgreSQL 加密 Trace 评价字段为事实源，消息缓存加载与保存时必须移除旧版 `feedbackText`。新 Run 同步保存可 reconciliation 的运行评分与加密 Trace 评价文字，历史消息保存 Trace 评价；已提交的新 Run 评分仍可通过原按钮撤销或切换，提交失败必须保留可重试状态并给出真实文字提示。
...
- 聊天文档完成解析后只显示一张可展开资料卡：文件名、大小、状态、展开预览和移除入口不得重复渲染；解析成功由卡片和固定说明表达下一步，不显示会遮挡手机内容的冗长 toast。
...
- 真实 `CgaAssessment` 的每题先按服务端返回的量表 ID、定义版本和题目 ID 匹配预录制 WAV；只有缺失或加载失败才通过统一的 server-only TTS BFF 兜底。患者老年模式且“自动朗读”开启时，新题只自动尝试朗读一次；同一评估重新开始后才会再次尝试，避免重复播报。每题始终提供“朗读本题 / 暂停朗读 / 继续朗读 / 停止朗读”与进度；切换题目立即停止旧音频，绝不让上一题继续播放。合成失败只显示可理解的文字降级提示并保留重新朗读入口，不使用本地静音文件或浏览器公开凭证伪造播放成功。
- 对话朗读和 CGA 的实时 TTS 兜底共用 server-only TTS BFF：合成长回答使用统一 30 秒 deadline，用户点击“正在准备，点击取消”必须中止浏览器请求和上游合成；禁止把 URL、模型名、音色或 API Key 以 `NEXT_PUBLIC_*` 形式作为后备配置。
- 患者老年模式默认开启“自动朗读”，其状态持久化到应用设置；仅刚刚成功完成的 AI 回复在完成 0.5 秒后自动尝试朗读，CGA 新题则按上一条规则立即尝试。朗读信号开始后即消费，刷新或重新打开历史会话绝不播放旧消息。自动朗读失败保持静默，用户始终可使用同一控件手动重试；准备中、播放中、暂停后分别提供取消、暂停/继续和停止，系统只允许一个音频播放器处于活跃状态。
- 重新生成必须复用原用户消息的文本、图片和已注册上传文档 ID；文档仍作为本次输入上下文而非知识库证据，不得因重新生成而丢失文档依据或改为无资料回答。
- 重新生成采用服务端 `AnswerVersion` 替换语义，不删除旧回答。出现多个版本时显示上一版/下一版和当前位置；选择成功后使用同主体 Run 返回并经 Zod 校验的 Markdown 与 Citation 原子替换正文。访客版本切换不得依赖被禁止的账户历史接口，也不得只改版本标签而保留旧正文。
...
- 全局 toast 在手机宽度必须使用可读的整行宽度、正常中文换行和明确的“关闭”按钮；不得因 flex 收缩被压成逐字竖排，也不得用冗长成功提示替代页面内已有的稳定状态。
...
- 全局网络监听必须把真实 `online/offline` 状态同步到应用：断网时在页面顶端显示带文字、`role=status` 的高对比横幅，恢复后移除横幅并给出一次“网络已恢复连接”提示。患者老年模式下横幅高度不低于 48px、正文不低于 18px；不得把浏览器仍处于离线状态伪装成可继续提交。
- 页面渲染异常由应用级 `ErrorBoundary` 与 App Router `error.tsx` 共同兜底，只显示用户可理解的恢复说明和至少 48px 的“重试 / 刷新页面”文字按钮；生产日志不得输出可能含敏感内容的异常正文。聊天请求失败属于可恢复业务错误，不得依赖页面错误边界：失败消息必须原位提供“重试”，复用原问题、图片和已登记文档且不重复插入用户消息。
...
- 五大处方在完成信息收集后可生成有本地医学证据编号的 `needs_clinician_review` 草案：加载使用稳定文字和已执行时间、错误必须有可理解的恢复路径；不得以骨架屏、旋转/闪烁动画或本地示例报告伪造完成。草案必须清楚标示“待临床复核、不可自行执行”、证据来源和上传资料与循证证据的区别；DDI/Beers/剂量规则未配置时，药物部分只能呈现核对与监测事项。用药审查可生成 `medication-rules-v3` 的有限、来源可追溯规则产物：展示命中、语料指纹、未识别条目和覆盖范围；患者风险提示仅在结果末尾出现一次；有限规则未命中不得显示为安全。保存前前端先校验所有服务端标为必填的字段，聚合提示缺失项、在字段旁显示可读错误并把焦点移至首个缺失字段，绝不把明显空表单发送为后端 422。浏览器可保存恢复用 UUID，但不得把医疗信息作为本地事实缓存。
- 五大处方信息收集可附加 PDF、Word、Markdown 或文本资料：先经 MinerU（文本文件本地读取）提取，再登记为当前会话的私有资料并把加密文档 ID 写入该收集记录。UI 必须清楚区分“正在用 MinerU 解析”和“正在保存资料”，使用克制的 Codex 三点和真实等待时间，不闪烁、不假进度；资料是未来受控报告的输入及“上传资料依据”，不是本地医学知识库证据，移除“本次收集”不得删除原会话资料。
- 同一会话和收集类型的初始化请求必须在浏览器内合并；服务端也必须将并发重试安全地归并到同一记录，绝不因唯一键冲突向用户返回 500。重新打开既有表单时优先读取浏览器保存的 UUID，失效后才恢复启动流程。
- 欢迎页和输入栏不得把任何功能称为“正式处方”或“完整 Beers 审查”。医生端可使用“用药规则审查”，但必须同时呈现其有限规则、待临床复核和 Beers 未安装边界；CGA 保留其真实服务端量表入口与结果路径。
...
- 移动会话抽屉必须用 `SheetTitle` 提供“会话菜单”可访问名称并支持 `Escape` 关闭；Popup 只做位移动画，不对整层文字做透明度淡入，避免入场阶段产生短暂低对比。患者/访客老年模式的交互主色使用 `#075985`，白字对比度为 7.56:1；常态患者端仍使用权威规范中的 `#0EA5E9`。
