...
- CGA 入口只显示服务端量表目录和 `CgaAssessment` 的真实状态机；评估期间隐藏聊天输入和重复标题，防止用户把自由文本误当作量表答案。退出确认只保留带文字的取消与确认操作，患者老年模式下均为至少 48px 的可点击控件。
...
- **文档解析状态**：文件标签+工具卡片双重反馈（上传中→解析中→完成/失败）。解析后显示“发送时加入对话”或“已加入本次对话”；前端只交给 BFF/服务层登记及传递 UUID，禁止直接拼接 Markdown。后端负责所有权、会话、撤销和 Harness 隔离。
