...
- 五大处方对话最多补充 5 轮，并持续显示已完成与剩余轮次；每轮只承载服务端返回的一个简短问题。第 5 轮后仍不完整时必须停止继续调用对话接口，保留当前收集记录，并按服务端 `missing_required_fields` 展示手动补齐表单。表单通过既有 `PATCH clinical-intakes/{id}` 与 `expected_revision` 保存；只有服务端返回 `information_complete_pending_governance` 才能开始生成。发生 revision 冲突时重新读取收集记录、保留用户未提交的输入并提示核对，其他保存失败也必须保留输入和重试入口。
- 五大处方草案生成或从服务端恢复后，对话区必须持续提供带文字的“查看五大处方草案”入口；关闭右侧报告只收起面板，不得使草案失去重新打开路径。重新打开时从已通过前端 Schema 校验的草案重建报告，不重新调用模型，也不创建第二份草案。
- 医生授权复核界面必须把只读且不可覆盖的“模型初稿”与追加保存的“医生修订版”分区展示，并按最新 revision 显示“待复核 / 已通过 / 已退回”。通过和退回都必须填写复核意见；医生修订版只能绑定模型初稿已有的 evidence ID，新的复核记录不得覆盖或伪装成模型初稿。患者端不得出现医生审核操作。
