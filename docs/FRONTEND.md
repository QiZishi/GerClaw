...
- 五大处方在完成信息收集后可生成有本地医学证据编号的 `needs_clinician_review` 草案：加载使用稳定文字和已执行时间、错误必须有可理解的恢复路径；不得以骨架屏、旋转/闪烁动画或本地示例报告伪造完成。草案必须清楚标示“待临床复核、不可自行执行”、证据来源和上传资料与循证证据的区别；DDI/Beers/剂量规则未配置时，药物部分只能呈现核对与监测事项。用药审查可生成 `medication-rules-v1` 的有限、来源可追溯规则产物：展示命中、语料指纹、未识别条目和覆盖范围；Beers 为 `not_installed_no_licensed_source` 时必须显式说明“未执行”，有限规则未命中不得显示为安全。保存前前端先校验所有服务端标为必填的字段，聚合提示缺失项、在字段旁显示可读错误并把焦点移至首个缺失字段，绝不把明显空表单发送为后端 422。浏览器可保存恢复用 UUID，但不得把医疗信息作为本地事实缓存。
