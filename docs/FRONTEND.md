...
- 面向患者的工具状态使用可理解的中文动作名（如“核对健康记录”“检索医学知识”），不得直接暴露 `search_memory` 等内部工具标识；折叠的思考和工具详情必须视觉上完全收起，展开后才可查看受控详情。
...
- 患者老年模式默认开启“自动朗读回复”，其状态持久化到应用设置；仅最后一条成功完成的 AI 回复会在完成 0.5 秒后自动尝试朗读。自动朗读失败保持静默，用户始终可使用同一控件手动重试；准备中、播放中、暂停后分别提供取消、暂停/继续和停止，系统只允许一个音频播放器处于活跃状态。
...
- 每条活跃 AI 回答在输出框最上端只显示一个 Codex 风格三点运行指示，配合明确的阶段文字和从真实请求开始计算的“已用时 mm:ss”；不在每张工具卡重复旋转或脉冲。三点动画必须低频、克制，并在 `prefers-reduced-motion` 下静止；完成/失败后停止动画，状态变化通过语义化 `aria-live` 传达给辅助技术。
...
- 所有角色在手机宽度均使用带文字的“菜单”入口；医生工作台标题须为该入口预留左侧空间，并在窄屏隐藏重复的右侧产品标识，避免可访问但视觉上被标题遮挡的菜单按钮。
