...
1. 访客无需登录即可完成所有核心体验；账号用户可持久化并按角色进入患者端或医生端。
...
| 用户 | 主要任务 | 默认体验 | 数据边界 |
|---|---|---|---|
| 访客 | 健康咨询、语音、CGA、处方、文档、Skill | 可切换患者/医生演示端；患者端默认老年模式 | 短期匿名主体；核心功能可用，不要求注册 |
...
验收：访客仍可体验核心功能；账号模式不可切换越权角色；所有审批动作有 actor、时间、理由和版本。
