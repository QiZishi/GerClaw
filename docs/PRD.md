...
2. 临床建议优先使用可追溯证据，不给出确定性诊断，并显示免责声明与红旗症状就医提示；一般健康教育、系统能力说明和用户自述即使暂时没有引用也必须正常回答。
...
| 用户 | 主要任务 | 默认体验 | 数据边界 |
|---|---|---|---|
| 访客 | 患者端与医生端的全部自有对话、语音、CGA、处方、文档、检索、Memory、Skill 和服务配置 | 从登录页主动选择匿名进入；默认患者端和老年模式，可自由切换患者端/医生端 | 每次页面生命周期使用短期匿名主体；不恢复历史会话，运行记录仅用于受控 Trace/反馈/Bad Case |
...
输入文本/语音/图片/文档 → Privacy 边界脱敏与资源校验（不拦截用户问题）→ 会话、画像与已加载 Skill 组装 → 本地 RAG 优先 → 必要时联网搜索 → AgentScope ReAct → 输出治理 → SSE 渲染 → 答案后的独立引用区、免责声明、Trace、反馈。

验收：停止、超时、模型切换、工具失败、引用不可用均有明确终态；引用不可用不能阻塞已经生成的正文，需在答案中如实说明依据状态。校验、
...
验收：登录页始终是入口，访客可选择匿名进入患者端并自由切换医生端，所有自有功能无需登录；账号模式不可切换越权角色；所有审批动作有 actor、时间、理由和版本。
...
- `UI-07` API/schema 版本不兼容、checkpoint 恢复、审批等待、数据删除和授权撤回都提供明确用户状态，不允许白屏或无限 loading；回答执行失败时保留用户原问题和已经通过公共校验的内容，交付自然语言的安全降级结果，不得展示内部提示词、格式合同、校验过程或统一占位框架，具体原因留在生产日志。
