...
- **流式中断容错**：浏览器 SSE transport 中断只分离 consumer，不能冒充用户取消；owner turn 继续在 lease/fencing 下持久化 RunEvent。账户客户端按 Run ID 和最后 sequence 重放并跟随同一 Run，重复 sequence 忽略、跨 Run 游标 fail closed。Provider 自身中断仍按执行失败处理，已接收片段不得冒充完成回答，也禁止跨模型续写。
...
- **Trace 时长投影**：所有事件在写入 `TraceEventCreate` 前通过同一有界投影转换 monotonic elapsed time；负值、非有限值归零，超过公共 24 小时上限则封顶。运行预算仍在业务边界强制执行；该投影只确保异常长驻进程的失败或取消终态不会被遥测 schema 二次拒绝。
...
- **终态事务原子性**：成功路径将 assistant 消息、AnswerVersion、回答组 current 指针、AgentRun completed、唯一 `done/completed` RunEvent、model/RAG/safety/agent-finish 事件与 completed Trace 在同一 request-scoped `AsyncSession` 中一次 commit；regeneration 在最终写入时再次校验 expected current version。失败路径在 session 行锁和 Redis owner 复验仍成立时，将 `SYSTEM_ERROR`、failed/cancelled Trace 与 Bad Case 一次 commit；任一 terminal write、fencing 或版本校验失败都会 rollback，禁止出现消息、版本、Run、Trace 和 Bad Case 相互矛盾的部分终态。
- **启动恢复互斥**：启动恢复器与 worker lease 使用同一 Redis Lua 互斥协议。恢复器取得带 TTL 的 recovery guard 后才扫描并锁定无主 `running/waiting_for_user` Run，并在数据库事务期间按 owner token 持续续租、commit 前复验；guard 存在时 worker 不能取得 lease，worker lease 存在时恢复器不能取得 guard。guard 丢失或 Redis 异常均 rollback、fail closed，禁止使用 `EXISTS` 检查后再更新数据库的竞态流程。
- **中断恢复与重放**：账户会话可查询当前主体唯一 recoverable Run，按 `after_sequence` 重放有界公开 RunEvent。仍为 `running` 的 Run 通过 GET stream 从最后 sequence 继续跟随；仅无已提交回答、Trace 仍为 running 的 `interrupted` Run 才显式 POST resume。服务端从加密持久化输入精确重建文本、Skill、附件和 regeneration 身份，缺失或损坏材料 fail closed；regeneration 从 source Run 解析原输入，不错误要求新旧 Trace 相等。已接收片段不得冒充完成回答。
...
- **SSE 背压与终态**：每连接使用有界队列和 heartbeat；`done` 只在 assistant 消息与 completed Trace 的原子事务提交后发出。客户端断开后 consumer-detached 信号立即释放可能阻塞的 queue publish，后续公开事件仍先持久化为 RunEvent、但不再堆积到无人消费的内存队列；producer 继续执行，客户端通过轮询式 GET stream 重连。工具终态、`error`/`cancelled` 和 queue sentinel 使用有界强制入队；显式取消先完成 cancelled Trace 原子事务，再公开 `cancelled`，前端收到该事件后才退出 stopping 状态。
