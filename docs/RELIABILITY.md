...
- **启动恢复互斥**：启动恢复器与 worker lease 使用同一 Redis Lua 互斥协议。恢复器取得带 TTL 的 recovery guard 后才扫描并锁定无主 `running/waiting_for_user` Run；guard 存在时 worker 不能取得 lease，worker lease 存在时恢复器不能取得 guard。Redis 异常 fail closed，禁止使用 `EXISTS` 检查后再更新数据库的竞态流程。
- **中断恢复与重放**：账户会话可查询当前主体唯一 recoverable Run，按 `after_sequence` 重放有界公开 RunEvent，并显式恢复同一 Run/Trace。仅无已提交回答、Trace 仍为 running 的 `interrupted` Run 可恢复；服务端从加密持久化输入精确重建文本、Skill、附件和 regeneration 身份，缺失或损坏材料 fail closed。浏览器 transport 断开仍取消当前 producer；只有进程崩溃或启动恢复形成的 `interrupted` Run 进入恢复协议，已接收片段不得冒充完成回答。
