...
| 场景 | 超时时间 | 说明 |
|------|---------|------|
| LLM单候选 API/完整流 | 30s（可配置） | 首 token 或持续心跳均不能绕过完整 stream deadline；无公开输出时切换备用模型 |
...
- `/health/ready`：就绪检查（readiness）— PostgreSQL/Redis/Qdrant/AgentScope/本地知识库、RAG 索引和独立 Memory collection 是否可用，返回200/503；Memory 探针绕过热路径 ready cache，强制复验 schema，并在 collection 被外部删除时有锁重建，禁止缓存误报
...
- **Memory 一致性**：每个用户通过 `health_profiles` 行锁串行画像合并，fact key 唯一约束去重，事实 revision 单调递增。Qdrant 在数据库 commit 前写入 revision-fenced point；数据库回滚最多留下无 PHI 孤儿点，检索的 PostgreSQL current-revision allowlist 会排除它。清理先 scroll 快照再按显式 point ID 删除，避免并发新 revision 被宽 filter 误删。
