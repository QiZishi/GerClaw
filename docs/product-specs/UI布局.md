...
| F-013 | 左侧边栏-底部区域 | 用户头像区（访客/医生/患者）、模式切换按钮（仅访客）、老年模式开关（仅患者端，默认开启）、主题切换按钮 | P0 | 点击头像展开菜单；游客可在医生/患者模式间切换，账号角色不提供跨端切换；老年模式开关即时生效不刷新 |
...
interface UIPreferences {
  ...
  userRole: 'doctor' | 'patient'; // 仅访客模式可切换，账号角色固定
}
