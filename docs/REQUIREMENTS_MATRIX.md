...
| AI-02 | Memory/健康画像引擎 | `modules/memory` | 加密、跨会话、冲突、无 PHI vector | ✅ 2026-07-17 隔离 external/integration 用例实测模型抽取、跨会话召回、数据库密文与 Qdrant PHI-free payload；另有 4 条 `memory-extraction-case-v1` 合成回归复用生产证据守卫，覆盖自述确认、否定撤销、他人信息与无证据拒绝，结果不回显输入或候选内容；医生授权和生命周期属于后续 IAM/DATA 闭环 |
| AI-03 | Skill 生命周期 | `modules/skill`、Skill UI | 注册/版本/隔离/viewer/自然语言生成与受控迭代/安全/真实模型 | ✅ 2026-07-17 隔离 external/integration 用例实测模型草稿、复核注册、会话加载、AgentScope 查看器、本地证据事件与 Trace；另有 BFF 接通的“生成待审阅修订”：仅自定义 Skill、revision 匹配、同 ID 且更高版本，结果不会自动保存或启用。`skill-draft-case-v1` 已以 3 条合成回归固定 `skill-draft-quality-v1` 的输入核对、本地证据、红旗和免责声明提示；医疗业务发布审核和外部模型持续质量评测仍属临床闭环 |
...
| AI-08 | Provider capability/version | services/adapters | schema/version/能力协商与不兼容拒绝 | 🚧 三模型 AgentScope 链现有服务端版本化 capability 声明：图片、工具调用、结构化输出不兼容的 slot 在外发前跳过，并向 PHI-free 临床 Trace 写入 capability version/拒绝码；production 缺任一声明即拒绝启动。Voice 以 `voice-capabilities-v1` 声明流式 ASR 与 PCM16 TTS；Search 以 `search-capabilities-v1` 在构建 Runtime 前拒绝不支持严格结构化结果的 AnySearch/Tavily，RAG 以 `rag-capabilities-v1` 在创建 HTTP client 前拒绝不支持批量 embedding 或 relevance-score rerank 的 Provider，MinerU 以 `mineru-capabilities-v1` 在 owner-bound egress 许可前拒绝不支持异步解析或 Markdown 导出的 Provider，并把版本写入 PHI-free 审计。Search/RAG readiness 同样返回版本。仍缺对 Provider 实时协商响应、统一 adapter 版本兼容窗口和运行中 capability 变更告警 |
...
| IAM-05 | 患者授权生命周期 | consent/RBAC/cache | 授予/到期/撤回，缓存与链接失效 | 🚧 `consent` 已提供患者→指定活跃医生的 revision-fenced、到期失效、可撤回授权；医生仅可通过有效 `health_profile_read`、`cga_report_read` 或 `prescription_draft_review` 读取相应健康画像、已完成 CGA 摘要或五大处方草案并追加复核。草案复核意见加密且绑定草案内容指纹，不能转为可执行处方；撤回/过期/不存在统一 404。患者菜单已提供仅限草案复核的医生代码授权、期限选择和撤回，医生端按患者代码读取已授权草案并提交意见；受限 BFF 不向匿名会话开放任何授权/复核端点。尚缺医生资质、患者可用的医生目录、其余资源授权、缓存失效和完整审计保留 |
...
| OPS-02 | metrics/feedback/eval/Bad Case | metrics/trace/feedback/eval | API、存储、回放、趋势 | 🚧 Trace/基础反馈与加密 Bad Case 已有；2026-07-18 管理员可通过 `account:admin` 读取 PHI-free 队列元数据并更新处置状态，管理员前端已实际接入，API/BFF 均不暴露 snapshot/评论/原始输入。队列查询显式不加载加密 snapshot，历史密钥不可读也不会阻断队列。28 个合成安全/输出安全/隐私/用药规则/Skill 草稿/Memory 提取 policy case 不回放真实输入；test image 的显式 opt-in RAG 评测对五个本地老年医学主题命中和一个无证据例均为 6/6。尚缺授权脱敏晋升、模型/临床评测、趋势与指标闭环 |
...
| OPS-07 | 供应链/SBOM | locks/CI/images | 固定版本、audit、SBOM、许可证、升级策略 | 🚧 2026-07-18 当前 `security` gate 已通过 Python Bandit/pip-audit、production API image 构建及 Python runtime CycloneDX SBOM；前端 `npm audit --omit=dev --audit-level=high` 不阻断，但仍报告 Next 传递 PostCSS 的 2 个中危告警，必须在发布前复审。Debian/npm runtime 清单、法务复审与发布签名仍待最终交付阶段。 |
...
| SEC-01 | red-team 与安全 eval | security-evaluation/eval/bad-case | 攻击语料、可执行风险档案、复现、回放、修复回归 | 🚧 Chat 的 `search_knowledge`、`search_memory`、`web_search` 已在 Runtime 注册/构建时经过版本、风险、网络、数据类别与必需控制的 fail-closed 档案门禁；外网工具还需服务端脱敏证明。`standard`、`cga`、`companion`、`prescription` workflow 也已在 Chat 前核验精确风险档案和适用的输入/输出/预算/非信任数据、患者归属、外发脱敏与证据 provenance 控制。老年医学/陪伴 Agent、Memory、local RAG corpus 与 loaded Skill 已在构造/AgentScope activation 边界接入生产档案门禁；统一攻击语料与回放仍缺 |
