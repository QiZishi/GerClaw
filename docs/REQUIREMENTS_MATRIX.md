...
| SEC-01 | red-team 与安全 eval | security-evaluation/eval/bad-case | 攻击语料、可执行风险档案、复现、回放、修复回归 | 🚧 Chat 的 `search_knowledge`、`search_memory`、`web_search` 已在 Runtime 注册/构建时经过版本、风险、网络、数据类别与必需控制的 fail-closed 档案门禁；外网工具还需服务端脱敏证明。`standard`、`cga`、`companion`、`prescription` workflow 也已在 Chat 前核验精确风险档案和适用的输入/输出/预算/非信任数据、患者归属、外发脱敏与证据 provenance 控制。其余 Agent/Skill/Memory/RAG-source 尚未接入生产档案门禁，统一攻击语料与回放仍缺 |
