...
| DATA-01 | 统一 schema/version/兼容 | domain/Zod/API/validation | version、兼容窗口、迁移、unknown fields | 🚧 `modules/validation` 已以 `public-chat-sse-v1` 实际接管 Harness→Chat Service→浏览器 SSE 的事件级严格校验；`local-rag-evidence-v1` 已在 Hybrid RAG 产出、AgentScope adapter、引用投影和 RAG Eval 复用，未知或不完整 provenance 不能被默认位置补写。五大处方生成/聊天式信息提取、Memory 抽取及 Skill 生成/修订已接入共享版本化模型输出验证，各自只接受显式 schema version。HTTP、工具、voice 与 export 仍需按风险逐步迁移，尚无跨边界统一兼容窗口 |
