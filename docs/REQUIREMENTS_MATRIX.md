...
| IAM-05 | 患者授权生命周期 | consent/RBAC/cache | 授予/到期/撤回，缓存与链接失效 | 🚧 `consent` 已提供患者→指定活跃医生的 revision-fenced、到期失效、可撤回授权；医生仅可通过有效 `health_profile_read`、`cga_report_read` 或 `prescription_draft_review` 读取相应健康画像、已完成 CGA 摘要或五大处方草案并追加复核。草案复核意见加密且绑定草案内容指纹，不能转为可执行处方；撤回/过期/不存在统一 404。尚缺医生资质、患者可用的医生目录/分享流程、告警等资源授权、BFF/前端、缓存失效和完整审计保留 |
