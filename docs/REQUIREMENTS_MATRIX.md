...
| AI-08 | Provider capability/version | services/adapters | schema/version/能力协商与不兼容拒绝 | 🚧 三模型 AgentScope 链现有服务端版本化 capability 声明：图片、工具调用、结构化输出不兼容的 slot 在外发前跳过，并向 PHI-free 临床 Trace 写入 capability version/拒绝码；production 缺任一 slot 显式声明时拒绝启动。ASR/TTS/Search/MinerU/embedding/rerank 的统一 capability 协商与契约仍待补齐 |
