...
def upgrade() -> None:
    op.create_table(
        ...
        sa.CheckConstraint("resource_scope IN ('health_profile_read','cga_report_read')", name="valid_patient_access_grant_resource_scope"),
        sa.CheckConstraint("status IN ('active','revoked')", name="valid_patient_access_grant_status"),
        sa.CheckConstraint("revision > 0", name="positive_patient_access_grant_revision"),
        sa.PrimaryKeyConstraint("id"),
        sa.UniqueConstraint("tenant_id", "patient_actor_id", "doctor_actor_id", "resource_scope", name="uq_patient_access_grants_subject_doctor_resource"),
    )
    op.create_index("ix_patient_access_grants_tenant_id", "patient_access_grants", ["tenant_id"])
    op.create_index("ix_patient_access_grants_patient_actor_id", "patient_access_grants", ["patient_actor_id"])
    op.create_index("ix_patient_access_grants_doctor_actor_id", "patient_access_grants", ["doctor_actor_id"])
    op.create_index("ix_patient_access_grants_doctor_active", "patient_access_grants", ["tenant_id", "doctor_actor_id", "status", "expires_at"])
