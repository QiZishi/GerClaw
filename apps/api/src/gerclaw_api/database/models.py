...
class IdentitySecurityEvent(Base):
    ...
    __table_args__ = (
        CheckConstraint(
            (
                "event_type IN "
                "('register','login','refresh','logout','password_change',"
                "'deactivate','admin_update','bootstrap')"
            ),
...
class ProviderEgressEvent(Base):
    ...
    __table_args__ = (
        CheckConstraint(
            "purpose IN "
            "('external_search_query','external_tts','external_asr_audio','external_document_parse','external_model_prompt')",
...
class PrescriptionDraftReview(Base):
    ...
    amended_markdown: Mapped[str | None] = mapped_column(EncryptedText(), nullable=True)
    amendment_evidence_ids: Mapped[list[str] | None] = mapped_column(EncryptedJSON(), nullable=True)
    amended_content_sha256: Mapped[str | None] = mapped_column(String(64), nullable=True)
