...
Harness 之前重新验证并规范化文本（Unicode NFKC、换行统一、去首尾空白、
重复附件引用）；在 SSE 终态前重新验证 `AgentResponse`，只投影公开
文本、引用和安全决定。内部 `structured`、模型/工具状态、prompt 或计费元数据不能
通过该模块进入浏览器或语音通道。

该模块不调用 Provider、不解析文件、不作医疗判断。附件字节/MIME 验证、文档解析、
ASR/TTS 和临床 workflow 分别仍由其专属服务边界负责。Chat 正文和语音公开文本不在
此模块设置人为短文本上限，公共 DTO 统一负责资源容量；语音音色为空时由适配器使用默认音色。

## 维护与演进

**可安全改进。** 可新增版本化公开 DTO、Unicode 规则或输出格式；领域解释、附件二进制和 provider adapter 仍应留在其模块。每次新字段先写严格 schema、浏览器 Zod 与兼容/淘汰计划。

**不可破坏的契约。** 输入规范化必须在 Trace/持久化/Harness 之前，公开输出必须在 SSE 之前复验；不得让 `structured`、prompt、工具状态、计费或模型内部字段穿过此边界。附件 UUID 去重不得由客户端绕过。
