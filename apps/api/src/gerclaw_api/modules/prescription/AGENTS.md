...
- `prescription-draft` first resolves the complete private input, short-circuits deterministic red flags, retrieves local evidence, accepts validated online-search evidence when available, and binds same-session uploaded materials as patient evidence. It requests strict, explicitly versioned `five-prescription-model-output-v1` output and writes server-owned evidence IDs/locators. `prescription-intake` model extraction likewise requires `prescription-intake-model-output-v1`. A missing, stale or malformed model-output version may only take the existing bounded fallback/baseline path. It always returns `needs_clinician_review` and a fixed disclaimer.
- Generic conversation history may receive only the owner-scoped boolean `has_prescription_draft` restore hint. It must never receive encrypted draft contents, intake answers, document/image IDs, or clinician-review text through that metadata path.
- A consented doctor may append an evidence-bound Markdown amendment to a review, but may never mutate the encrypted model draft. The amendment must cite one or more evidence IDs already present in that exact draft; it is encrypted, content-hashed, revisioned per doctor, and visible only through the existing owner/consented-doctor review projection. It remains clinical review material, not an electronic prescription or an execution command.
...
- Doctor amendment changes require regression coverage for immutable base content, missing/unknown evidence rejection, grant revocation, doctor-local revision ordering and patient restore.
