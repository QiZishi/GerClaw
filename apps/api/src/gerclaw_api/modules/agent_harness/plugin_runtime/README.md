...
web search tools; `ApprovalCoordinator` validates and durably parks AgentScope ASK requests.
It does not activate a second plugin system, and existing GerClaw modules remain the only
capability owners.
...
Consumers: the current run-lifecycle executor, future planner, and user capability selection.
