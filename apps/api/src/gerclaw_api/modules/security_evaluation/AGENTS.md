...
workflow registry (`standard`, `cga`, `companion`, `prescription`), the core
Runtime assets (geriatric/companion Agents, encrypted Memory, local RAG
corpus), and every loaded declarative Skill; none may enter Runtime without a
matching active profile and applicable controls.
...
- A profile is server-owned, strict, version-bound and keyed by exact asset
  ...
  alter a profile.
- Runtime tool/workflow/Agent/Memory/RAG-source/Skill capability risk, network
