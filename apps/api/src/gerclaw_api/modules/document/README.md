...
1. The browser sends a file only to the Next.js MinerU BFF. Before the external MinerU submit call, that BFF resolves the same HttpOnly principal used by the GerClaw BFF and creates an owner-bound `external_document_parse` `prepared` audit decision. It replaces the original filename with an opaque UUID-based name. The decision never contains a filename, Markdown, size or page count, and is completed as `succeeded` or `failed` after the provider outcome; the document bytes remain necessary parsing input.
