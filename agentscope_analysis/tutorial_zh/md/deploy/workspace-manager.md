<!-- 来源：https://docs.agentscope.io/versions/2.0.6dev/zh/deploy/workspace-manager -->

# 工作区管理

**工作区管理器**（Workspace Manager）持有隔离策略，并管理智能体服务中每个智能体的 [`Workspace`](https://docs.agentscope.io/versions/2.0.6dev/zh/building-blocks/workspace/overview) 实例的生命周期。它是 `agentscope.workspace` 模块在服务侧的对应实现 —— 每一种 `WorkspaceBase` 子类都有一个配套的 `WorkspaceManagerBase` 的子类来负责创建、缓存与删除。

它的特点：

- 可配置的隔离粒度 PER_AGENT PER_SESSION PER_USER (user_id, agent_id, session_id)
- 可插拔的沙箱后端 create_app
...
> **提示** 想深入了解 workspace *究竟是什么*（文件系统布局、gateway、MCP 接线、内置工具），请看 [Workspace](https://docs.agentscope.io/versions/2.0.6dev/zh/building-blocks/workspace/overview) 章节。
...
> **提示** 在 session 创建请求中显式指定 `workspace_id` 永远会覆盖策略。这就是内置团队工具（`AgentCreate` / `AgentInvite`）用来让子 agent 与团队 leader 共用同一个 workspace 的机制。
...
- Router /session /workspace/* Depends(get_workspace_manager) manager.get_workspace(...)
- ChatService get_workspace
- 内置团队工具 AgentCreate AgentInvite assign_workspace_id(...)
...
1. **创建 session** —— `/session` router 只计算一次 workspace id 并把它写到 session 记录上： ```python
...
2. **后续每个请求**带着这个 session —— 无论是 chat 运行、MCP 注册还是 skill 上传 —— 都会从 session 记录里读出保存好的 `workspace_id`，然后请求 manager 把它落到具体的 workspace 上： ```python
...
> **提示** 由于 `workspace_id` **只在 session 创建时生成一次**并持久化，运行中的部署改动 `isolation` 策略并**不会**对现存 session 重新分组 —— 它们会继续保留当初分配的 id。只有新 session 才会走新策略。
...
任何基于 [`WorkspaceBase`](https://docs.agentscope.io/versions/2.0.6dev/zh/building-blocks/workspace/overview) 构建的 workspace 类，都可以通过配一个 `WorkspaceManagerBase` 子类的方式接到智能体服务里。大多数情况下只需要写一小段准备 + 缓存的样板代码 —— 隔离策略由基类自动继承。
...
> **提示** 如果后端需要一个后台 sweeper 来做空闲驱逐，或者需要跑长时间的准备逻辑，重写 `__aenter__` / `__aexit__` 来启停这些机制 —— `DockerWorkspaceManager` 的源码就是一个完整示例。
...
- E2BWorkspaceManager
- DaytonaWorkspaceManager agentscope.workspace.id
- OpenSandboxWorkspaceManager agentscope.workspace.id list_sandbox_infos(...)
- K8sWorkspaceManager

> **提示** 本地开发用 `LocalWorkspaceManager`，单机生产用 `BubblewrapWorkspaceManager` 或 `DockerWorkspaceManager`，一旦要横向扩容就切到 E2B、Daytona、OpenSandbox 或 K8s。
