<!-- 来源：https://docs.agentscope.io/versions/2.0.6dev/zh/deploy/agent-service -->

# 架构

智能体服务（Agent Service）是基于 FastAPI 把 AgentScope 的智能体转化为**多租户（Multi-tenant）、多会话（Multi-session）的 HTTP 服务**。它接管智能体*外围*的全部职责 —— 请求路由、按用户的资源生命周期、会话（Session）状态、持久化、调度（Schedule），以及工具调用的卸载，让基于 [`Agent`](https://docs.agentscope.io/versions/2.0.6dev/zh/building-blocks/agent/overview) 编写的代码无需重写即可承接生产流量。
...
| 能力 | 说明 |
| --- | --- |
| 智能体团队（Agent Team） | Leader 智能体派生 worker 智能体，并通过内置 team 工具协调它们；详见 [Agent Team](https://docs.agentscope.io/versions/2.0.6dev/zh/deploy/agent-team) 章节。 |
...
> **提示** 本服务**不**自带用户鉴权系统。它提供 `X-User-ID` header 占位依赖，由开发者替换为自己的鉴权中间件（JWT、OAuth、session token 等）。
...
![后台工具卸载与唤醒演示](https://mintcdn.com/agentscope-ai-786677c7/c8Op3M8_8-WXpeVH/images/bg_tool.gif?s=ecba271192d28e8d36ef6ba02ef5cc7f)

后台任务卸载 —— 长耗时工具被转入后台，结果到达后唤醒智能体，对话从中断处自然续上。

![权限系统 bypass 模式演示](https://mintcdn.com/agentscope-ai-786677c7/c8Op3M8_8-WXpeVH/images/permission_bypass.gif?s=45da293c3c8b1b819f2e87924652d563)

bypass 模式下的权限系统 —— 智能体端到端运行，不为工具调用暂停确认。

![任务规划演示](https://mintcdn.com/agentscope-ai-786677c7/c8Op3M8_8-WXpeVH/images/task.gif?s=d9bf05e01cde9c044cfe558a4de67d2f)

任务规划 —— 智能体把复杂工作拆成可追踪的计划，并随执行不断更新。

![智能体团队协作演示](https://mintcdn.com/agentscope-ai-786677c7/c8Op3M8_8-WXpeVH/images/team.gif?s=604f394f94cca0dbca8fdeee75a0f7ff)
...
以 TTL 缓存方式管理工作区（文件存储、MCP server、skill）。内置 `LocalWorkspaceManager` 按智能体隔离；其他策略见[工作区实现与隔离](#%E5%B7%A5%E4%BD%9C%E5%8C%BA%E5%AE%9E%E7%8E%B0%E4%B8%8E%E9%9A%94%E7%A6%BB)。
...
团队中子智能体创建的可复用蓝图。每个模板定义了一个子智能体*类型*（例如 `"researcher"`、`"coder"`），预设了系统提示词、权限上下文与任务上下文。注册后，`AgentCreate` 工具会暴露 `subagent_type` 参数，使 leader 智能体可以路由到相应的模板。详见[自定义子智能体类型](https://docs.agentscope.io/versions/2.0.6dev/zh/deploy/agent-team#%E8%87%AA%E5%AE%9A%E4%B9%89%E5%AD%90%E6%99%BA%E8%83%BD%E4%BD%93%E7%B1%BB%E5%9E%8B)。
...
> **注意** 默认的 `X-User-ID` header 不提供任何鉴权。生产部署前请替换为真实的鉴权方案 —— 见[用户鉴权](#%E7%94%A8%E6%88%B7%E9%89%B4%E6%9D%83)。
...
若智能体需要超出内置范围的工具，向会话的工作区附加 MCP client 与 skill。开箱即用的情况下，每个智能体已经能访问工作区的内置工具（文件系统、shell、搜索……）、任务规划工具、调度与后台任务控制工具，以及 —— 当会话是团队 leader 或成员时 —— [Agent Team](https://docs.agentscope.io/versions/2.0.6dev/zh/deploy/agent-team) 中描述的团队协调工具。通过 `create_app` 的 `extra_agent_tools` 传入的工具也会一并合入。
...
智能体服务中的每次操作都归属于从请求中解析出的 `user_id`。在该边界之下，服务管理七类资源 —— 六类持久化资源（图左侧）加把它们运行时行为串起来的消息总线（图右侧）。若要让凭证、智能体或知识库跨越用户之间的这条边界，见[资源共享](https://docs.agentscope.io/versions/2.0.6dev/zh/deploy/sharing)。
...
> **提示** 要记住的形态：**智能体是可复用的模板，会话是运行时状态的承载单位**，而消息总线则是当外部事件（调度、队友、后台工具）有话要说时把空闲会话唤醒的桥梁。
...
按会话流端点（`GET /sessions/{id}/stream`）通过 SSE 输出 AgentScope 原生的 [`AgentEvent`](https://docs.agentscope.io/versions/2.0.6dev/zh/building-blocks/message-and-event) 流。要让同一智能体服务于不同前端协议，安装协议中间件拦截 SSE 流并改写每帧。
...
> **注意** 默认的 `X-User-ID` header 不提供鉴权。生产部署前请始终替换为安全机制。

### 工作区实现与隔离

可独立配置两条正交维度：

- 工作区后端 LocalWorkspace DockerWorkspace E2BWorkspace
- 隔离策略 LocalWorkspaceManager agent_id WorkspaceManagerBase get_workspace
...
- InboxMiddleware HintBlock HintBlockEvent
- ToolOffloadMiddleware
- StateChangeMiddleware tasks_context permission_context CustomEvent
