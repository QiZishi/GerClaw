<!-- 来源：https://docs.agentscope.io/versions/2.0.6dev/zh/building-blocks/permission-system/tool-check -->
...
| 检查 | 作用 | 生效模式 |
| --- | --- | --- |
| [只读判定](#%E5%8F%AA%E8%AF%BB%E5%91%BD%E4%BB%A4) | 解析每一次调用，自动放行事实上只读的调用 | 所有模式 |
| [危险路径保护](#%E5%8D%B1%E9%99%A9%E8%B7%AF%E5%BE%84%E4%BF%9D%E6%8A%A4) | 对触及敏感文件的操作发出不可绕过的安全 ASK | `DEFAULT` / `ACCEPT_EDITS`（`DONT_ASK` 下转为 DENY；`BYPASS` 下跳过） |
...
> **注意** 针对以下路径的操作在 `DEFAULT`、`ACCEPT_EDITS`、`DONT_ASK` 下触发不可绕过的 ASK（`DONT_ASK` 下转为 DENY）。`BYPASS` 模式按设计跳过此检查；如果在 BYPASS 下仍需要危险路径保护，请为具体路径添加拒绝规则。
