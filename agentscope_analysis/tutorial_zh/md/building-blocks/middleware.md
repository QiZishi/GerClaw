<!-- 来源：https://docs.agentscope.io/versions/2.0.6dev/zh/building-blocks/middleware -->
...
> **提示** 以上 hook 均作用于**智能体**层面。若需要在单个工具实例上挂钩 —— 无论该工具是在智能体内部还是外部被调用 —— 请参见[工具中间件](https://docs.agentscope.io/versions/2.0.6dev/zh/building-blocks/tool/python-tool#%E5%B7%A5%E5%85%B7%E4%B8%AD%E9%97%B4%E4%BB%B6)。

三种类型的差别：

- Onion next_handler()
- Transformer
- Tool source Agent.__init__ list_tools()
...
> **提示** 当前 `on_acting` 只包裹智能体运行时内部的工具执行；通过 external execution 在智能体外部执行的工具不会被 `on_acting` 追踪到。
...
| 长期记忆 | `AgenticMemoryMiddleware`， `Mem0Middleware` | 提供跨会话的长期记忆实现 |
...
> **提示** Agent 收到 `ExternalExecutionResultEvent`（外部执行的工具结果）时，`TracingMiddleware` 会为每条外部执行结果合成一个补偿 span，让外部系统执行的工具也能保留完整可观测性。
...
1. 在智能体上下文最后一条 assistant 消息中追加一个 HintBlock AssistantMsg
2. 把下一步 reasoning 的 tool_choice ToolChoice(mode="none")
...
> **提示** 该中间件**实例自身无状态** —— 所有运行时状态存放在 `agent.state.middle_context` 中，以中间件 key 与当前 `reply_id` 作为索引。这意味着同一个中间件实例可以安全地在多个智能体间共享，且预算状态在 human-in-the-loop（HITL）中断与恢复中保持有效。reply 结束时状态会被自动清理。

> **提示** 预算的作用域是**单次 reply**，而非智能体生命周期。每次新的 reply 都会从零计数，因此该限制对每次 `agent(...)` 调用独立生效。
...
AgentScope 以中间件的形式支持长期记忆，让智能体能够在多个会话间持久保存并调取信息。详见 [长期记忆](https://docs.agentscope.io/versions/2.0.6dev/zh/building-blocks/long-term-memory) 章节。

### RAG

AgentScope 中知识库同样以中间件形式提供，允许智能体在推理时访问外部知识库。详见 [RAG](https://docs.agentscope.io/versions/2.0.6dev/zh/building-blocks/rag) 章节。
