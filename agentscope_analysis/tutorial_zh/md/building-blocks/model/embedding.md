<!-- 来源：https://docs.agentscope.io/versions/2.0.6dev/zh/building-blocks/model/embedding -->
...
| `embedding_cache` | `EmbeddingCacheBase \| None` | 可选缓存，跳过重复的 API 调用（参见[嵌入缓存](#%E5%B5%8C%E5%85%A5%E7%BC%93%E5%AD%98)） |
| `context_size` | `int` | 单条输入的最大 token 数 |
| `max_retries` | `int` | 每个批次在可重试错误下的最大重试次数 |
| `retry_delay` | `float` | 两次重试之间的间隔秒数 |

> **提示** `dimensions` 的合法取值因模型而异：每张模型卡片通过顶层的 `dimensions` 字段固定默认值，通过 `supported_dimensions` 声明允许的取值（例如 `text-embedding-v4` 接受 2048 / 1536 / 1024 / … / 64）。参见 [Embedding 模型卡片](#embedding-%E6%A8%A1%E5%9E%8B%E5%8D%A1%E7%89%87)。
...
2. 所有批次通过 asyncio.gather 并发
3. 每个批次在 API 专属的可重试错误下独立重试，最多 max_retries
4. 结果合并为单个 EmbeddingResponse
...
> **提示** 多模态模型用**内容感知分批**取代简单的批大小切分：输入会按贪心策略打包，确保每个批次满足模型对总元素数、图片数、视频数的单次请求限制（例如 `qwen3-vl-embedding` 单次允许 20 个元素 / 5 张图片 / 1 个视频，`tongyi-embedding-vision-plus` 允许 20 / 64 / 8）。开发者无需自行拆分输入。
...
新增嵌入模型 API 的步骤与[自定义大语言模型 API](https://docs.agentscope.io/versions/2.0.6dev/zh/building-blocks/model/llm#%E8%87%AA%E5%AE%9A%E4%B9%89%E6%A8%A1%E5%9E%8B-api)一致。
...
`EmbeddingModelCard` 是通用[模型卡片](https://docs.agentscope.io/versions/2.0.6dev/zh/building-blocks/model/overview#%E5%89%8D%E7%AB%AF%E9%9B%86%E6%88%90)在嵌入场景下的对应物，带有嵌入专属的默认值：输出类型 `application/x-embedding` 表示该模型产出稠密向量。
