<!-- 来源：https://docs.agentscope.io/versions/2.0.6dev/zh/building-blocks/model/tts -->
...
- stream=False TTSResponse
- stream=True AsyncGenerator[TTSResponse, None] is_last=True
...
> **提示** `DashScopeRealtimeTTSModel`（Qwen3）以 token 级别粒度产出音频，每次 `push()` 通常都能返回音频数据。而 `realtime=True` 的 `DashScopeCosyVoiceTTSModel` 依赖 CosyVoice 服务端自动分句后才进行合成，音频只在检测到完整句子边界后才返回，因此 `push()` 对不完整的句子可能返回空响应。调用 `synthesize()` 会强制合成所有剩余文本（包括未完成的句子）。
...
在智能体层，语音合成通过 [`TTSMiddleware`](https://docs.agentscope.io/versions/2.0.6dev/zh/building-blocks/middleware#ttsmiddleware) 集成，它会拦截智能体的文本输出并自动合成语音：
...
`TTSModelCard` 描述语音合成模型的能力（可用音色、流式支持与参数范围），其 schema 与覆盖机制与通用的[模型卡片](https://docs.agentscope.io/versions/2.0.6dev/zh/building-blocks/model/overview#%E5%89%8D%E7%AB%AF%E9%9B%86%E6%88%90)一致。每张卡片由模型实现旁的 YAML 文件定义，下面的 tab 分别展示各模型 API 的一张卡片：
