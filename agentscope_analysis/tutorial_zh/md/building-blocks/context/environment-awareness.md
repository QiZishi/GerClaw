<!-- 来源：https://docs.agentscope.io/versions/2.0.6dev/zh/building-blocks/context/environment-awareness -->
...
| `extra_fields` | `{}` | 附加到每次注入的自定义字段（参见[注入自定义字段](#%E6%B3%A8%E5%85%A5%E8%87%AA%E5%AE%9A%E4%B9%89%E5%AD%97%E6%AE%B5)） |
| `emit_hint_event` | `True` | 注入发生时是否产出 `HintBlockEvent` |

> **提示** `context_buffer_ratio` 必须小于[上下文配置](https://docs.agentscope.io/versions/2.0.6dev/zh/building-blocks/context/compress-context#%E9%85%8D%E7%BD%AE%E5%8E%8B%E7%BC%A9)的 `trigger_ratio`，以保证上下文用量在压缩发生之前注入。否则智能体构造函数会抛出 `ValueError`。
