<!-- 来源：https://docs.agentscope.io/versions/2.0.6dev/zh/building-blocks/tool/manage-tools -->
...
- "basic"
- 每次调用 reset_tools 整体覆盖 True
- 被激活的工具组，其 instructions
- 未被激活的工具以及 skill 不会出现在大模型调用的输入中。

> **注意** 元工具的输入表示所有工具组的**最终状态**。任何未显式置为 `True` 的工具组都会被停用，无论之前的状态如何。
