...
describe('GerClaw domain plugins', () => {
  ...
  it('keeps chronic trends isolated by disease, metric and unit in the task runtime', async () => {
    const runtime = await source('task-runtime-local/src/index.ts')
    expect(runtime).toContain('trend.conditionId === item.conditionId')
    expect(runtime).toContain('trend.metricLabel === item.metricLabel')
    expect(runtime).toContain('trend.unit === item.unit')
  })
...
describe('GerClaw integration plugin contracts', () => {
  ...
  it('RAG composes isolated shared and account-private dsh-library providers without Tool-as-API', async () => {
    ...
    const runtimeProvider = await source('library-dsh-runtime/src/index.ts')
    ...
    expect(profile).toContain('name: "@gerclaw/library-dsh-runtime"')
    ...
    expect(profile).toContain('gerclawLibraryRuntime: gerclaw-shared-rag')
    expect(privateProvider).toContain('extends GerclawLibrary')
    expect(privateProvider).toContain("static inject = ['gerclawLibraryRuntime']")
    expect(runtimeProvider).toContain('new LibraryStore(')
    expect(runtimeProvider).toContain('this.ctx.storageDomain.open(libraryDomainSpec)')
    ...
    for (const text of [privateProvider, shared, rag]) {
      expect(text).not.toContain('new LibraryStore(')
  ...
  it('profile-bundle loads native Plan/Goal and every GerClaw provider through Cordis', async () => {
    ...
    const agent = await readFile(join(root, '../../apps/cli/config/agent-presets/gerclaw/agent.cordis.yml'), 'utf8')
    ...
    expect(await source('library-dsh-runtime/package.json')).toContain('dsh-library')
  })
  ...
  it('app is GerClaw-only, chat-centred, and delegates seven exports to the artifact service', async () => {
    ...
    expect(client).toContain("ctx.locale.setLocale('zh')")
    expect(client).not.toContain("querySelectorAll<HTMLElement>('body *')")
    expect(client).not.toContain('MutationObserver')
    expect(navigation).not.toContain("{ id: 'chat'")
    ...
    expect(app).not.toContain("append('turn/start'")
    expect(app).not.toContain("append('turn/end'")
    expect(app).not.toContain("append('agent/message'")
  ...
  it('protects the native chat prescription flow, brand layout and sidebar order', async () => {
    ...
    const app = await source('client/src/index.ts')
    expect(client).toContain('sendConversationMessage(')
    expect(client).toContain('我想开始一份新的五大处方')
    expect(app).toContain("status: 'not_started' as const")
    ...
    expect(css).toContain("[data-chat-flow-kind='command']")
    expect(css).toContain("[data-chat-flow-kind='tool-call']")
    expect(css).toContain("[data-variant='think']")
    expect(css).toContain('button:has([data-gerclaw-brand-name])')
    expect(css).toContain('min-height: 40px')
    expect(css).toContain('grid-template-rows: 18px 14px')
    expect(css).toContain('overflow: visible !important')
    expect(css).toContain('@media (prefers-reduced-motion: reduce)')
    expect(css).toContain('outline-offset: 2px')
    expect(await source('client-ui/src/client/Modal.tsx')).toContain("event.key !== 'Tab'")
  })
